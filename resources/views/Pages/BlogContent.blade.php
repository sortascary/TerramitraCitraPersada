<x-layout>
    @vite(['resources/js/blog.js'])
    <main class="pt-20 px-10" style="max-width: 1200px; margin: 0 auto; min-height: 100vh">
        <div class="flex items-center gap-3">
            <button onclick="location.href='/Blog'">
                <svg class="w-9 h-9 text-black" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
                    <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 12h14M5 12l4-4m-4 4 4 4"/>
                </svg>
            </button>
            <h1 style="margin: 30px 0">{{ $blog->title }}</h1>
        </div>
        <p>{{ DATE_FORMAT($blog->created_at, 'd M Y ')}}</p>
        
        @if ($blog->contentAttachments->count() >0)
            <div id="default-carousel" class="relative w-full" data-carousel="slide">
                <div class="relative h-80 overflow-hidden rounded-base md:h-140">
                    @foreach ($blog->contentAttachments as $attachment)
                        <a  href="{{ asset($attachment->file) }}" data-fancybox="blog-{{ $blog->id }}" data-carousel-item>
                            <img
                                src="{{ asset($attachment->file) }}"
                                class="h-full w-full object-cover"
                                alt="{{ $attachment->id }}"
                            >
                        </a>
                    @endforeach
                </div>

                @if ($blog->contentAttachments->count() > 1)
                    
                <button type="button" class="absolute top-0 start-0 z-30 flex items-center justify-center h-full px-4 cursor-pointer group focus:outline-none" data-carousel-prev>
                    <span class="inline-flex items-center justify-center w-10 h-10 rounded-base bg-white/30 dark:bg-gray-800/30 group-hover:bg-white/50 dark:group-hover:bg-gray-800/60 group-focus:ring-4 group-focus:ring-white dark:group-focus:ring-gray-800/70 group-focus:outline-none">
                        <svg class="w-5 h-5 text-white rtl:rotate-180" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24"><path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m15 19-7-7 7-7"/></svg>
                        <span class="sr-only">Previous</span>
                    </span>
                </button>
                <button type="button" class="absolute top-0 end-0 z-30 flex items-center justify-center h-full px-4 cursor-pointer group focus:outline-none" data-carousel-next>
                    <span class="inline-flex items-center justify-center w-10 h-10 rounded-base bg-white/30 dark:bg-gray-800/30 group-hover:bg-white/50 dark:group-hover:bg-gray-800/60 group-focus:ring-4 group-focus:ring-white dark:group-focus:ring-gray-800/70 group-focus:outline-none">
                        <svg class="w-5 h-5 text-white rtl:rotate-180" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24"><path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m9 5 7 7-7 7"/></svg>
                        <span class="sr-only">Next</span>
                    </span>
                </button>
                @endif
            </div>
        @endif
        <div class="py-5">
            {!! nl2br(e($blog->desc)) !!}
        </div>
        
        <hr class="my-5">
        <div class="flex flex-col item-center justify-center text-center">
            <h3>share to:</h3>
            <div class="flex item-center justify-center">
                <a class="whatsapp" target="blank">
                    <svg class="w-6 h-6 text-gray-800 dark:text-black" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
                        <path fill="currentColor" fill-rule="evenodd" d="M12 4a8 8 0 0 0-6.895 12.06l.569.718-.697 2.359 2.32-.648.379.243A8 8 0 1 0 12 4ZM2 12C2 6.477 6.477 2 12 2s10 4.477 10 10-4.477 10-10 10a9.96 9.96 0 0 1-5.016-1.347l-4.948 1.382 1.426-4.829-.006-.007-.033-.055A9.958 9.958 0 0 1 2 12Z" clip-rule="evenodd"/>
                        <path fill="currentColor" d="M16.735 13.492c-.038-.018-1.497-.736-1.756-.83a1.008 1.008 0 0 0-.34-.075c-.196 0-.362.098-.49.291-.146.217-.587.732-.723.886-.018.02-.042.045-.057.045-.013 0-.239-.093-.307-.123-1.564-.68-2.751-2.313-2.914-2.589-.023-.04-.024-.057-.024-.057.005-.021.058-.074.085-.101.08-.079.166-.182.249-.283l.117-.14c.121-.14.175-.25.237-.375l.033-.066a.68.68 0 0 0-.02-.64c-.034-.069-.65-1.555-.715-1.711-.158-.377-.366-.552-.655-.552-.027 0 0 0-.112.005-.137.005-.883.104-1.213.311-.35.22-.94.924-.94 2.16 0 1.112.705 2.162 1.008 2.561l.041.06c1.161 1.695 2.608 2.951 4.074 3.537 1.412.564 2.081.63 2.461.63.16 0 .288-.013.4-.024l.072-.007c.488-.043 1.56-.599 1.804-1.276.192-.534.243-1.117.115-1.329-.088-.144-.239-.216-.43-.308Z"/>
                    </svg>
                </a>
                <a class="facebook" target="blank">
                    <svg class="w-6 h-6 text-gray-800 dark:text-black" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" viewBox="0 0 24 24">
                        <path fill-rule="evenodd" d="M13.135 6H15V3h-1.865a4.147 4.147 0 0 0-4.142 4.142V9H7v3h2v9.938h3V12h2.021l.592-3H12V6.591A.6.6 0 0 1 12.592 6h.543Z" clip-rule="evenodd"/>
                    </svg>
                </a>
                <a class="copy">
                    <svg class="w-6 h-6 text-gray-800 dark:text-black " aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
                        <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 4h3a1 1 0 0 1 1 1v15a1 1 0 0 1-1 1H6a1 1 0 0 1-1-1V5a1 1 0 0 1 1-1h3m0 3h6m-5-4v4h4V3h-4Z"/>
                    </svg>
                </a>


            </div>
        </div>
        <hr class="my-5">

        <h4 class="mb-4">Komentar ({{ $comments->count() }})</h4>

        @auth        
            <div class="comment-box d-flex gap-3 mb-5">
                
                @if (Auth::user()->image != null)
                    <img 
                        src="{{ asset(Auth::user()->image) }}"
                        alt="Your avatar"
                        class="rounded-circle"
                        style="width: 48px; height: 48px; object-fit: cover;"
                    >
                @else                    
                    <img 
                        src="/Storage/profiles/User.png"
                        alt="Your avatar"
                        class="rounded-circle"
                        style="width: 48px; height: 48px; object-fit: cover;"
                    >
                @endif

                <form class="flex-grow-1" action="{{ route('comment', $blog->id) }}" method="POST">
                    @csrf
                    <textarea 
                        class="form-control"
                        rows="3"
                        placeholder="Tulis komentar..."
                        style="resize: none;"
                        id="text" name="text"
                    ></textarea>

                    <div class="d-flex justify-content-end mt-2">
                        <button type="submit" class="btn btn-primary btn-sm">
                            Send
                        </button>
                    </div>
                </form>
            </div>
        @endauth

        @guest
            <div class="px-3 py-10 my-5 bg-gray-100 rounded text-sm text-gray-600">
            <a href="/login" class="text-blue-600 font-semibold">
                Login
            </a>
            atau
            <a href="/register" class="text-blue-600 font-semibold">
                daftar
            </a>
            untuk menulis komentar.
        </div>
        @endguest

        
        
        
        <div class="row">
            @foreach ($comments as $comment)
                <div class="comment d-flex gap-3 mb-4">
                    @if ($comment->user->image != null)
                        <img 
                            src="{{ asset($comment->user->image) }}" 
                            alt="User avatar"
                            class="rounded-circle"
                            style="width: 48px; height: 48px; object-fit: cover;"
                        >
                    @else
                        <img 
                            src="/Storage/profiles/User.png" 
                            alt="User avatar"
                            class="rounded-circle"
                            style="width: 48px; height: 48px; object-fit: cover;"
                        >
                    @endif
                    
        
                    <div>
                        <div class="d-flex align-items-center gap-2">
                            <h6 class="mb-0 fw-bold">{{ $comment->user->name }}</h6>
                            <span class="text-muted small">{{ $comment->created_at }}</span>
                        </div>
        
                        <p class="mt-2 mb-0">
                            {{ $comment->text }}
                        </p>
                    </div>
                </div>
            @endforeach
        </div>



    </main>
    <x-footer></x-footer>
</x-layout>